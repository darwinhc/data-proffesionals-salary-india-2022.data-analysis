...
```{r, echo=F}
who_use_associate <- subset(company_job, grepl("Associate", jobs)); who_use_associate
```
...
```{r}
gb_company <- df %>%group_by(df$Company.Name) %>% 
  summarise(mean_ts=mean(total_salaries_by_job_title, na.rm=T), .groups = "drop")
gb_company <- gb_company[order(-gb_company$mean_ts),]

gb_company$rank <- 1:length(gb_company$mean_ts)
colnames(gb_company) <- c("")
knitr::kable(head(gb_company[,c("rank", "df$Company.Name", "mean_ts")], 25))
````    
